...
// 操作的用户信息
    ...
    String synNodeid = Util.null2String(weaver.file.Prop.getPropValue("DMD", "synNodeid"));
    boolean hassyn = false;
    if (synNodeid.indexOf("," + currentnodeid + ",") > -1) hassyn = true;
...
//RecordSet.executeProc("workflow_currentoperator_SByUs",userid+""+flag+usertype+flag+requestid+"");
    ...
    if (isremark != 0 && isremark != 1 && isremark != 5 && isremark != 9 && isremark != 7) {
        System.out.println("--264------mrnf-");
        System.out.println("--264------isremark-=====" + isremark);
        ...
        if (isremark == 1 || isremark == 9) {
            ...
            if (IsCanSubmit || coadCanSubmit) {
                ...
                if ("".equals(ifchangstatus)) {
                    ...
                    //RCMenuHeight += RCMenuHeightStep ;
                    if (hassyn) {
                        strBar += "{text: '同步数据',iconCls:'btn_submit',handler: function(){SynDMD();}},"; //同步数据
                    }
...
<input type=hidden name=picInnerFrameurl id="picInnerFrameurl"
       value="/workflow/request/WorkflowRequestPictureInner.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&fromFlowDoc=<%=fromFlowDoc%>&modeid=<%=modeid%>&requestid=<%=requestid%>&workflowid=<%=workflowid%>&nodeid=<%=nodeid%>&isbill=<%=isbill%>&formid=<%=formid%>&randpara=<%=System.currentTimeMillis()%>">
<input type=hidden name=statInnerFrameurl id="statInnerFrameurl"
       value="WorkflowRequestPicture.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&hasExt=true&requestid=<%=requestid%>&workflowid=<%=workflowid%>&nodeid=<%=nodeid%>&isbill=<%=isbill%>&formid=<%=formid%>&randpara=<%=System.currentTimeMillis()%>">
<input type=hidden name=reqeustresourceframeurl id="reqeustresourceframeurl"
       value="/workflow/request/RequestResourcesTab.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&requestid=<%=requestid%>&workflowid=<%=workflowid%>&nodeid=<%=nodeid%>&isbill=<%=isbill%>&formid=<%=formid%>&workflowid=<%=workflowid%>&reportid=<%=reportid%>&isfromreport=<%=isfromreport%>&isfromflowreport=<%=isfromflowreport%>&iswfshare=<%=iswfshare%>">
<input type=hidden name=workflowshareurl id="workflowshareurl"
       value="/workflow/request/WorkflowSharedScope.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&requestid=<%=requestid%>&wfid=<%=workflowid%>">
<!--
...
<script language="JavaScript">
    ...
    function SynDMD() {
        jQuery.ajax({
            url: "/integration/DMD/synData.jsp?requestid=<%=requestid%>&nodeid=<%=currentnodeid%>&workflowid=<%=workflowid%>&tt=" + Math.random(),
            async: false,
            cache: false,
            success: function doSuccess(data) {
