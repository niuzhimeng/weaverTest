 ...
 * 
 */
public class MailRemind extends BaseAction {
	private static BaseBean bb = new BaseBean();
	// 公司发文workflowid
	private static String strCompDispatchWfid = bb.getPropValue("wnrequestinfo",
			"strCompDispatchWfid");
	// 公司发函workflowid
	private static String strCompAletterWfid = bb.getPropValue("wnrequestinfo",
			"strCompAletterWfid");
	// 公司会议纪要workflowid
	private static String strCompMeetingWfid = bb.getPropValue("wnrequestinfo",
			"strCompMeetingWfid");
	// 部门发文workflowid
	private static String strDeptDispatchWfid = bb.getPropValue("wnrequestinfo",
			"strDeptDispatchWfid");
	// 部门发函workflowid
	private static String strDeptAletterWfid = bb.getPropValue("wnrequestinfo",
			"strDeptAletterWfid");
	// 部门会议纪要workflowid
	private static String strDeptMeetingWfid = bb.getPropValue("wnrequestinfo",
			"strDeptMeetingWfid");
	// 授权流程workflowid
	private static String strAuthorWfid = bb.getPropValue("wnrequestinfo",
			"strAuthorWfid");
	// 规章制度workflowid 
	private static String strSystemWfid = bb.getPropValue("wnrequestinfo",
			"strSystemWfid");
	...
	public String execute(RequestInfo request) {
		...
		// ----------授权流程
			...
			LogUtil.debugLog("====授权流程邮件提醒查询信息==========>"+SelectSql);
			rs.execute(SelectSql);
			if (rs.first()) {
	...
	public static String getHrefDocHtmlNames(String docids, String label ,int num) {
		...
		if(num < 0){
			...
			for (int i = 0; i < strarray.length; i++) {
				LogUtil.doWriteLog("==strarray[i]===="+strarray[i]);
