...
public class WfCheckBudget extends BaseAction {
    ...
    public String execute(RequestInfo request) {
        ...
        LogUtil.doWriteLog("==预结算流程节点后验证Start==" + requestid);
        // 流程表名
        String strTablename = "";
        // 获取表名
        ...
        if (!"S".equals(strDate)) {
            ...
            strOrderCode = strs[0];
            strNum = strs[1];
            String errMes = "本流程中存在重复 工单号:" + strOrderCode + ",序号:" + strNum + ",请检查本流程填写数据。";
            LogUtil.doWriteLog(errMes);
            request.getRequestManager().setMessageid("12508");
            request.getRequestManager().setMessagecontent(errMes);
            return "0";
        } else {
            ...
            if (!"S".equals(strDate)) {
                ...
                strOrderCode = strs[0];
                strNum = strs[1];
                String errMes = "历史流程中存在重复 工单号:" + strOrderCode + ",序号:" + strNum + ",请检查本流程填写数据。";
                LogUtil.doWriteLog(errMes);
        ...
        LogUtil.doWriteLog("==预结算流程节点后验证End==" + requestid);
        return "1";
    }
