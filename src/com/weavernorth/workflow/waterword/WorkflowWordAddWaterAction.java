 ...
 * 
 * @author llh
 * @create 2018-01-05 10:19
 **/
public class WorkflowWordAddWaterAction implements Action {
	...
	public String execute(RequestInfo request) {
		...
		// 合同签订流程
		if (workFlowId.equals("137")||workFlowId.equals("237")) {
			...
			if (rs.next()) {
				// 本地上传正文
				String bdsczw = Util.null2String(rs.getString("bdsczw"));
				if (!"".equals(bdsczw)) {
					String[] bdsczws = bdsczw.split(",");
					for (int i = 0; i < bdsczws.length; i++) {
						String bdsczw1 = bdsczws[i];
						...
						object.put("creater",
								DocUtil.getDocCreaterLoginId(bdsczw1));
						object.put("fileName", DocUtil.getFileName(bdsczw1));
						object.put("directory",
								DocUtil.getDocDirectory(bdsczw1));
						array.add(object);
					}
				}
				// 合同正文文档
			...
			LogUtil.debugLog("==合同签订array=>" + array.toString());

		}
		// 合同变更流程
		if (workFlowId.equals("144")) {
			String selSql = "select htzwfj,htzwwd,htfj from " + tableName
					+ " where requestid=" + requestId;
			rs.executeSql(selSql);
				...
				if (!"".equals(htzwwd)) {
					...
					if (!"".equals(htfj)) {
						...
						for (int i = 0; i < htfjs.length; i++) {
							...
							object.put("fileName", DocUtil.getFileName(htfj1));
							object.put("directory",
									DocUtil.getDocDirectory(htfj1));
							array.add(object);
						}
					}
				}
			}
			LogUtil.debugLog("==合同变更array=>" + array.toString());
		}
		// 循环解压图片
		if (array.size() > 0) {
			/*int index0 = 0;
			...
			for (int i = 0; i < array.size(); i++) {
				...
				if (fileRealPath.contains("doc")|| fileRealPath.contains("docx")|| fileRealPath.contains("pdf")) {
					...
					if (result) {
						...
						/*LogUtil.debugLog("==index0===>" + index0);
	...
	public static void main(String[] args) throws DocumentException,
			IOException {
		String fileRealPath = "D:/WEAVER/ecology/filesystem/201801/Y/分页符.doc";
		String newFilePath = fileRealPath.substring(
