 ...
 * HR-07 调动（借调）审批流程 - 退回
 */
public class ChangeFlowTh extends BaseAction {

    private BaseBean baseBean = new BaseBean();

    @Override
    public String execute(RequestInfo requestInfo) {
        baseBean.writeLog("HR-07 调动（借调）审批流程 - 退回 start" + TimeUtil.getCurrentTimeString());
        ...
        try {
            ...
            baseBean.writeLog("HR-07 调动（借调）审批流程 - 退回 end" + TimeUtil.getCurrentTimeString());
        } catch (Exception e) {
            baseBean.writeLog("HR-07 调动（借调）审批流程 - 退回 Exception: " + e);
