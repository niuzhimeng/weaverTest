 ...
 * 发票退回并变更发票状态-分摊多发票
 */
public class InvoiceRejectFentanDuofp extends BaseAction {
    ...
    public String execute(RequestInfo requestInfo) {
        ...
        try {
            ...
            this.writeLog("发票退回并变更发票状态-分摊多发票 End ===============");
        } catch (Exception e) {
            this.writeLog("发票退回并变更发票状态-分摊多发票 异常： " + e);
            requestInfo.getRequestManager().setMessageid("110000");
            requestInfo.getRequestManager().setMessagecontent("发票退回并变更发票状态-分摊多发票 异常： " + e);
            return "0";
        }
