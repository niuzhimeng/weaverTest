 ...
 * 2019-05-10 修改取值
 */
public class Jbsq0510 extends BaseAction {
    /**
     * 流程编号
     */
    private String bh = "";
    ...
    public String execute(RequestInfo requestInfo) {
        ...
        this.writeLog("01-总部员工加班申请0510修改版 start requestid --- " + requestId + "  operatetype --- " + operatetype + "   fromTable --- " + tableName);
        if ("submit".equals(operatetype)) {
            try {
                ...
                if (recordSet.next()) {
                    // 操作类型 MOD DEL INS
                    String lclx = recordSet.getString("lclx");
                    String czlxStr = "INS";
                    if ("1".equals(lclx)) {
                        czlxStr = "MOD";
                    ...
                    // 流程编号
                    bh = recordSet.getString("bh");
                    // 拼接对象
                    DT_HR0002_ININPUT dt_hr0002_ininput = new DT_HR0002_ININPUT();
                    // 设置接口类型 '2002'."人事公出；'2001'."人事请假；'2005'."人事加班； '2010'."补贴次数
                    dt_hr0002_ininput.setTYPE("2005");
                    ...
                    dt_hr0002_ininputpt2002.setBEGDA(changeDays(recordSet.getString("jbrq"))); // 开始日期

                    dt_hr0002_ininputpt2002.setENDDA(changeDays(recordSet.getString("jbrq"))); // 结束日期

                    dt_hr0002_ininputpt2002.setBEGUZ(recordSet.getString("kssj").replace(":", "") + "00"); // 开始时间
                    dt_hr0002_ininputpt2002.setENDUZ(recordSet.getString("jssj").replace(":", "") + "00"); // 结束时间
