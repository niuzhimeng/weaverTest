 ...
 * 定时触发【JT_转正提醒】流程
 */
public class ZzTriggerFlow extends BaseCronJob {
    ...
    @Override
    public void execute() {
        baseBean.writeLog("定时触发【JT_转正提醒】 Start： " + TimeUtil.getCurrentTimeString());
        ...
        try {
            ...
            String standardSql = "SELECT\n" +
                    ...
                    "\th.departmentid,\n" +
                    "\th.probationenddate,\n" +
                    "\tc." + JiaJieConfigInfo.zzrq.getValue() + ",\n" +
                    "\tc." + JiaJieConfigInfo.RZRQ.getValue() + " \n" +
                    "FROM\n" +
                    "\tHRMRESOURCE h\n" +
                    "\tLEFT JOIN CUS_FIELDDATA c ON h.id = c.id \n" +
                    "\tAND c.SCOPEID = 3 \n" +
                    "WHERE\n" +
                    "\th.subcompanyid1 = " + JiaJieConfigInfo.ZONG_BU_ID.getValue() + " \n" +
                    "\tAND h.STATUS IN ( '0', '1' ) \n" +
                    "\tAND ( c." + JiaJieConfigInfo.zzrq.getValue() + " IS NULL OR c." + JiaJieConfigInfo.zzrq.getValue() + " = '' )";
            baseBean.writeLog("JT_转正提醒，查询sql: " + standardSql);
            ...
            while (mainRecordSet.next()) {
                ...
                triggerSet.executeQuery("select id from " + JiaJieConfigInfo.TRIGGER_TABLE_NAME.getValue() +
                        " where person_name = '" + id + "' and flow_type = '1'");
                if (triggerSet.next()) {
                    continue;
                }
                // 入职日期
                String rzrq = Util.null2String(mainRecordSet.getString(JiaJieConfigInfo.RZRQ.getValue()));

                String dateAdd = TimeUtil.dateAdd(rzrq, 160);
                if (currentDateString.equals(dateAdd)) {
                    ...
                    mainField[i].setFieldValue(mainRecordSet.getString(JiaJieConfigInfo.RZRQ.getValue())); // 入职日期
                    mainField[i].setView(true);
                    mainField[i].setEdit(true);
                    ...
                    workflowBaseInfo.setWorkflowId(JiaJieConfigInfo.JT_ZZ_FLOW_ID.getValue());// 流程id
                    ...
                    workflowRequestInfo.setRequestName("JT_转正提醒" + TimeUtil.getCurrentTimeString());// 流程标题
            ...
            addCount(flowPerMap);
            baseBean.writeLog("【JT_转正提醒】创建流程完毕===============requestid-person :" + JSONObject.toJSONString(flowPerMap));
        } catch (Exception e) {
            baseBean.writeLog("定时触发【JT_转正提醒】 Error: " + e);
        }
        baseBean.writeLog("定时触发【JT_转正提醒】 End： " + TimeUtil.getCurrentTimeString());
    }
