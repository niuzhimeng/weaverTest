...
public class CompanyYc extends BaseAction {
    ...
    public String execute(RequestInfo requestInfo) {
        ...
        recordSet.executeSql("SELECT  * FROM workflow_requestoperatelog WHERE requestid = '" + requestId + "' ORDER BY (operatedate + ' '+ operatetime) desc");
        String nodeid = "";//
        if (recordSet.next()) {
            nodeid = recordSet.getString("nodeid");
        }
        this.writeLog("上一个操作节点id为： " + nodeid);

        if ("2188".equals(nodeid)) { //2188 为第三节点id,防止重复更改编号
