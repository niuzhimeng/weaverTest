...
import java.util.*;
/**
*@program: currentMoneyForFilm
*@description: 现金借款单（影院/汉堡王）
*@author: chchq
*@updateDate: 2019/6/20
*/
public class currentMoneyForFilm extends BaseAction {
    ...
    public String execute(RequestInfo request) {
        ...
        this.writeLog("currentMoneyForFilm 现金借款单（影院/汉堡王）--请求id："+requestid+"--start");
        if ("submit".equals(operator)) {
            try {
                ...
                //String payDept = rs.getString("fycdcode");//费用承担部门----payDept---不传
                ...
                String oaBizType = "现金借款单(影院/汉堡王)";//requestname---oaBizType
                ...
                String recAmtValue = rs.getString("sqhjje");//核定合计金额---hdhjje改为申请合计金额---sqhjje
                ...
                detailRs.execute("select * from formtable_main_246_dt1  where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表信息" + "select * from formtable_main_246_dt1  where mainid=" + id + "");
                while (detailRs.next()) {
        ...
        this.writeLog("currentMoneyForFilm 现金借款单（影院/汉堡王）--请求id："+requestid+"--end");
        return result;
    }
}
