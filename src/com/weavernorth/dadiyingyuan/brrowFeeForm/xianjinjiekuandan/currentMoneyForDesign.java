...
import java.util.*;
/**
*@program: currentMoneyForDesign
*@description: 现金借款单（设计/工程/放映/拓展）
*@author: chchq
*@updateDate: 2019/6/20
*/
public class currentMoneyForDesign extends BaseAction {
    @Override
    public String execute(RequestInfo request) {
        ...
        this.writeLog("currentMoneyForDesign 现金借款单（设计/工程/放映/拓展）--请求id："+requestid+"--start");
        ...
        if ("submit".equals(operator)) {
            try {
                ...
                //是否要传--先默认
                //String payDept = rs.getString("D005.034-03");//费用承担部门----payDept---不传
                String payOrgId = rs.getString("fyzfgs");//	费用支付公司----payOrg
                //String payType = rs.getString("zffs1");//支付方式----payType---不传
                String describ = rs.getString("sy");//事由
                String repaymentDate = rs.getString("yjhkr");//预计还款日---repaymentDate
                String oaBizType = "现金借款单(设计/工程/放映/拓展)";//requestname---oaBizType
                ...
                String recAmtValue = rs.getString("sqhjje");//核定合计金额hdhjje--改为---申请合计金额sqhjje
                String recBankAccount = rs.getString("yhzh");//银行账号
                ...
                detailRs.execute("select * from  formtable_main_248_dt1 where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表信息" + "select * from  formtable_main_248_dt1 where mainid=" + id + "");
                while (detailRs.next()) {
                    String approvedAmtValue = detailRs.getString("hdje");//核定金额
                    String expenseTypeId = detailRs.getString("fylx");//fylx---expenseType(新增字段)
                    ...
                    String approvedAmt = numberFormat2.format(approvedAmtV);
                    ...
                    String localAmt = numberFormat3.format(localAmtV);
                    ...
                    //新增--写死
                    feeExpenseList.setZhifu(zhiFu);//支付款类型D0040206003
                ...
                this.writeLog("请求id："+requestid+"，传递参数---->" + json);
        ...
        this.writeLog("currentMoneyForDesign 现金借款单（设计/工程/放映/拓展）--请求id："+requestid+"--end");
        return result;
    }
}
