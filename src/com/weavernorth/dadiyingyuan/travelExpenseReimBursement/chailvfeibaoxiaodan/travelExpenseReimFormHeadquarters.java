...
import java.util.*;
/**
*@program: travelExpenseReimFormHeadquarters
*@description: 差旅费报销单（总部/媒体）
*@author: chchq
*@updateDate: 2019/6/20
*/
public class travelExpenseReimFormHeadquarters extends BaseAction {
    @Override
    public String execute(RequestInfo request) {
        ...
        this.writeLog("travelExpenseReimFormHeadquarters 差旅费报销单（总部/媒体）--请求id："+requestid+"--start");
        ...
        if("submit".equals(operator)) {
            try {
                ...
                String oaBizType = "差旅费报销单(总部/媒体)";

                //明细表---expense
                String amtApprovedLocalValue = rs.getString("hdjezh12");//核定金额总和hdjezh12---amtApprovedLocal
                String amtApprovedNoRateLocalValue = rs.getString("sqjezh");//申请金额总和sqjezh---amtApprovedNoRateLocal
                //String approvedTaxAmt="";(不传)
                //String exchangeRate="1";(固定值1)
                String expenseTypeId = rs.getString("fylxn");//费用类型fylxn---expenseType
                //String  localAmt="";(不传)
                ...
                String recAmtValue = rs.getString("sfje");//实付金额sfje---recAmt

                String recBankAccount = rs.getString("yxzh");//银行账号yhzh---recBankAccount
                String recPerson = rs.getString("skrmc");//收款人名称skrmc---recPerson
                String recCity = rs.getString("khydq");//	开户行地区khydq---recCity
                //2019.4.4原借款金额
                ...
                String amtApprovedLocal = numberFormat1.format(amtApprovedLocalV);
                ...
                String otherAmt = numberFormat3.format(otherAmtV);
                ...
                String stayAmt = numberFormat4.format(stayAmtV);
                ...
                String transfee = numberFormat5.format(transfeeV);
                ...
                String transfer = numberFormat6.format(transferV);
                ...
                String recAmt = numberFormat7.format(recAmtV);
                ...
                String amountValue = "";
                detailRsTwo.execute("select * from formtable_main_241_dt2   where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表2信息" + "select * from formtable_main_241_dt2   where mainid=" + id + "");

                //明细表2信息
                while (detailRsTwo.next()) {//明细表2字段齐全
                    String loanNumberId = detailRsTwo.getString("jkdh");//流程单号lcdh---loanNumber
                    ...
                    amountValue = detailRsTwo.getString("cjkje");//冲借款金额cjkje---amount
                    ...
                    String balance = numberFormat8.format(balanceV);

                    //处理千分位问题
                    double amountV = Double.valueOf(amountValue.replace(",", "")).doubleValue()<0?0:Double.valueOf(amountValue.replace(",", "")).doubleValue();
                    NumberFormat numberFormat9= NumberFormat.getNumberInstance();
                    numberFormat9.setGroupingUsed(false); //设置了以后不会有千分位，如果不设置，默认是有的
                    String amount = numberFormat9.format(amountV);
                    ...
                    travelRevLoanBean.setLoanDate(loanDate);
                    travelRevLoanBean.setLoanReason(loanReason);
                    travelRevLoanBean.setExpenseType(expenseTypeTwo);
                    travelRevLoanBean.setBalance(balance);
                ...
                detailRs.execute("select * from formtable_main_241_dt1 where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表1信息" + "select * from formtable_main_241_dt1  where mainid=" + id + "");
                //明细表1信息
                while (detailRs.next()) {//明细表1字段齐全
                    ...
                    //新增字段--->用途--->yt
                    //String purpose = detailRs.getString("yt");//交通工具yt---purpose
                    ...
                    travelExpenseListBean.setLocalAmt(amtApprovedLocal);
        ...
        this.writeLog("travelExpenseReimFormHeadquarters 差旅费报销单（总部/媒体）--请求id："+requestid+"--end");
        return result;
    }
}
