...
import java.util.*;
/**
*@program: travelExpenseReimFormDesign
*@description: 差旅费报销单（设计/工程/放映/拓展）
*@author: chchq
*@create: 2019/6/20
*/
public class travelExpenseReimFormDesign extends BaseAction {
    ...
    public String execute(RequestInfo request) {
        ...
        this.writeLog("travelExpenseReimFormDesign 差旅费报销单（设计/工程/放映/拓展）--请求id："+requestid+"--start");
        if("submit".equals(operator)) {
            try {
                ...
                //String payDept = rs.getString("fycdcode");//费用承担部门fycdbm---payDept
                String payOrgId = rs.getString("fyzfgs");//费用支付公司fycdgs---payOrg
                //String payType = rs.getString("zffs2");//支付方式zffs---payType
                ...
                String oaBizType = "差旅费报销单(设计/工程/放映/拓展)";
                this.writeLog("---->差旅费报销单(设计/工程/放映/拓展)");

                //明细表字段--expense
                String amtApprovedLocalValue = rs.getString("hdjezh1");//核定金额总和hdjezh1---amtApprovedLocal
                String amtApprovedNoRateLocalValue = rs.getString("sqjezh");//申请金额总和sqjrzh---amtApprovedNoRateLocal
                //String approvedTaxAmt="";(不传)
                //String exchangeRate="1";(固定值1)
                String expenseTypeId = rs.getString("fylxn");//费用类型fylx---expenseType
                //String  localAmt="";(不传)
                ...
                String recAmtValue = rs.getString("sfje");//sfje	实付金额---recAmt
                String recBankAccount = rs.getString("yxzh");//银行账号yeah---recBankAccount
                ...
                detailRsTwo.execute("select * from formtable_main_242_dt2  where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表2信息" + "select * from formtable_main_242_dt2   where mainid=" + id + "");
                //明细表2信息
                while (detailRsTwo.next()) {//明细表2字段齐全
                    String loanNumberId = detailRsTwo.getString("jkdh");//流程单号jkdh---loanNumber
                ...
                detailRs.execute("select * from formtable_main_242_dt1  where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表1信息" + "select * from formtable_main_242_dt1  where mainid=" + id + "");
                //明细表1信息
                ...
                String s = oTSP.receiveOaTravel(json);
                this.writeLog("请求id："+requestid+"，返回结果---->" + s);
        ...
        this.writeLog("travelExpenseReimFormDesign 差旅费报销单（设计/工程/放映/拓展）--请求id："+requestid+"-- end");
        return result;
    }
}
