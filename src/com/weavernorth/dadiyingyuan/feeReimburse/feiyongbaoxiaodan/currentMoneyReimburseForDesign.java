...
import java.util.*;
/**
*@program: currentMoneyReimburseForDesign
*@description: 现金报销单（设计/工程/放映/拓展）
*@author: chchq
*@updateDate: 2019/6/21
*/
public class currentMoneyReimburseForDesign extends BaseAction {
    @Override
    public String execute(RequestInfo request) {
        ...
        this.writeLog("currentMoneyReimburseForDesign 现金报销单（设计/工程/放映/拓展）--请求id："+requestid+"--start");
        ...
        if("submit".equals(operator)) {
            try {
                ...
                String payOrgId = rs.getString("fyzfgsXY");//	费用支付公司
                //String payType = rs.getString("zffs1");//支付方式---payType
                ...
                String oaBizType = "现金报销单(设计/工程/放映/拓展)";//requestname---oaBizType
                ...
                //新增字段
                //String jiaona = rs.getString("jnd");//缴纳地N---jiaona
                String zhifu = rs.getString("zfklb");//支付款类型N---zhifu
                this.writeLog("实付金额------>" + recAmt);
                ...
                detailRs.execute("select * from formtable_main_251_dt1    where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表1信息" + "select * from formtable_main_251_dt1   where mainid=" + id + "");
                //明细表1信息
                ...
                while (detailRs.next()) {
                    ...
                    //String purpose =  "null".equals(detailRs.getString("yt")) ? "" : detailRs.getString("yt");//用途---purpose
                    this.writeLog("purpose--->" + purpose);
                ...
                //明细表2信息
                detailRsTwo.execute("select * from  formtable_main_251_dt2    where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表2信息" + "select * from  formtable_main_251_dt2   where mainid=" + id + "");
                while (detailRsTwo.next()) {
                   ...
                   /* //处理千分位问题
                    ...
                    String balance = numberFormat4.format(balanceV);
                    ...
                    String amount = numberFormat5.format(amountV);
                ...
                if ("1020000".equals(c)) {
                    ...
                    request.getRequestManager().setMessagecontent("金蝶数据保存异常，请联系相关人员处理，错误信息详情" + msg);
        ...
        this.writeLog("currentMoneyReimburseForDesign 现金报销单（设计/工程/放映/拓展）--请求id："+requestid+"--end");
        return result;
    }
}
