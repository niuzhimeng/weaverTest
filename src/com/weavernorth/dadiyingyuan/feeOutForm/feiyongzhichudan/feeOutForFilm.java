    ...
    public String execute(RequestInfo request) {
        ...
        if("submit".equals(operator)) {
            try {
                ...
                String recBankAccountId = rs.getString("gysyhzh");//供应商帐号---recBankAccount

                String retailCusId = rs.getString("khmc");//	客户code---retailCus
                String retailBank = recBank;
                String retailBankAccountId = rs.getString("khyhzh");//客户账号---retailBankAccount
                ...
                String oaBizType = "汇款申请单(影院/汉堡王)";//requestname---oaBizType
                ...
                //2019.4.4新增字段
                String zjyyId = rs.getString("zjyy");//影院---zjyy
                String longnumberYy = "";
                if ("null".equals(zjyyId) || zjyyId.trim().length() == 0) {
                    longnumberYy = "";
                } else {
                    //增加--影院
                    RecordSet yyID = new RecordSet();
                    String yyIdSql = "select longnumber from uf_hsxmlbmxb where id=" + zjyyId;
                    yyID.execute(yyIdSql);
                    yyID.next();
                    longnumberYy = yyID.getString("longnumber");
                    this.writeLog("影院----->" + longnumberYy);
                }
                String yajinId = rs.getString("yjxz");//押金性质N---yy---yajin
                String longnumberYj = "";
                if ("null".equals(yajinId) || yajinId.trim().length() == 0) {
                    longnumberYj = "";
                } else {
                    //增加押金
                    RecordSet yjID = new RecordSet();
                    String yjIdSql = "select longnumber from uf_hsxmlbmxb where id=" + yajinId;
                ...
                //明细表2信息
                detailRsTwo.execute("select * from  formtable_main_243_dt2 where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表2信息" + "select * from  formtable_main_243_dt2 where mainid=" + id + "");
                while (detailRsTwo.next()) {
                    ...
                    String shuifeiId = detailRsTwo.getString("kmda");//科目档案N---kmda---shuifei
                    this.writeLog("科目档案N--->" + shuifeiId);
                    this.writeLog("执行到这了？----》1");
                    //解决金额千分位问题
                    ...
                    this.writeLog("执行到这了？----》8");
                    //是否抵税
                    boolean dishui;
                    if (dishuiId.equals("0")) {
                        dishui = true;//是
                        this.writeLog("执行到这了？----》9");
                    } else {
                        dishui = false;//否
                    }
                    this.writeLog("执行到这了？----》10");
                    //科目
                    String shuifei = "";
                    ...
                    //新增2019.4.4
                    feeOutExpenseList.setZaijian(longnumberYy);
                    feeOutExpenseList.setYajin(longnumberYj);
                    feeOutExpenseList.setProduct(longnumberCp);
                    feeOutExpenseList.setGongcheng("");
                    feeOutExpenseList.setFeiyong("");
                    feeOutExpenseList.setZhifu(longnumber);
