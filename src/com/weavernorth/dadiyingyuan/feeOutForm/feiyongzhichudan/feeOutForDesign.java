...
import java.util.*;
/**
*@program: feeOutForDesign
*@description: 汇款申请单（设计/工程/放映/拓展）
*@author: chchq
*@create: 2019/6/20
*/
public class feeOutForDesign extends BaseAction {
    @Override
    public String execute(RequestInfo request) {
        //1、获取流程表单数据
        String requestid = request.getRequestid();//获取流程请求id
        String tableName = getPropValue("workFlowTableName","feeOutForDesign");
        ...
        this.writeLog("feeOutForDesign 汇款申请单（设计/工程/放映/拓展）--请求id："+requestid+"--start");
        ...
        if("submit".equals(operator)) {
            try {
                ...
                String recBankAccountId = rs.getString("gyszh");//供应商帐号---recBankAccount

                String retailCusId = rs.getString("khmc");//	客户code---retailCus
                String retailBank = recBank;
                String retailBankAccountId = rs.getString("khzh");//客户账号---retailBankAccount
                ...
                Double attendCount = rs.getDouble("fjs");//fjs	附件数
                String oaBizType = "汇款申请单(设计/工程/放映/拓展)";//requestname---oaBizType

                //新增字段
                String zaijianId = rs.getString("yyjd");//影院N---yy---zaijian
                ...
                if (recTypeId.equals("0")) {
                    ...
                    this.writeLog("客户--->" + "select code from uf_khb where id=" + retailCusId);
                ...
                //明细表2信息
                detailRsTwo.execute("select * from formtable_main_245_dt2  where mainid=" + id + "");//获取对应的明细表数据
                this.writeLog("明细表2信息" + "select * from formtable_main_245_dt2  where mainid=" + id + "");
                while (detailRsTwo.next()) {
                    ...
                    String payEntryDeptId = detailRsTwo.getString("fycdbmn");//费用承担部门---费用支付部门
                    String payEntryOrgId = detailRsTwo.getString("fycdgsn");//费用承担公司---费用支付公司
                    ...
                    String shuifeiId = detailRsTwo.getString("kmda");//科目档案N---km---shuifei
                    ...
                    this.writeLog("承担部门Id--->" + payEntryDeptId);
                    this.writeLog("承担公司Id--->" + payEntryOrgId);
                    //查询费用承担部门和承担公司code
                ...
                if ("1020000".equals(c)) {
                    ...
                    request.getRequestManager().setMessagecontent("金蝶数据保存出错,请联系相关人员处理。金蝶错误信息详情：" + msg);
                    ...
                    this.writeLog("请求id："+requestid+"，金蝶错误信息详情：" + msg);
                    request.getRequestManager().setMessagecontent("金蝶数据保存出错,请联系相关人员处理。金蝶错误信息详情：" + msg);
                    request.getRequestManager().setMessageid("11111"+requestid+"55555");
                    result = Action.FAILURE_AND_CONTINUE;
                    tsjg = "1";
        ...
        this.writeLog("feeOutForDesign 汇款申请单（设计/工程/放映/拓展）--请求id："+requestid+"--end");
        return result;
    }
}
