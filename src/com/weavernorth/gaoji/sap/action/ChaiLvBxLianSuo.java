 ...
 * 连锁差旅报销
 */
public class ChaiLvBxLianSuo extends BaseAction {
    ...
    public String execute(RequestInfo requestInfo) {
        ...
        this.writeLog("差旅报销节点后操作执行------表名： " + tableName + " 请求id： " + requestId);
        ...
        while (detailSet.next()) {
            ...
            if (detailSet.getString("je") == null || "".equals(detailSet.getString("je").trim())) {
                continue;
            }
            //明细表字段
            BaoXiaoDetailVo baoXiaoDetailVo = new BaoXiaoDetailVo();
            baoXiaoDetailVo.setZDJBH(djbh);//单据编号
            baoXiaoDetailVo.setZFYLX(detailSet.getString("xzbfylx"));//费用类型
            baoXiaoDetailVo.setZBX(detailSet.getString("je"));//报销金额
            baoXiaoDetailVo.setAUFNR(detailSet.getString("nbdd"));//费用承担项目
        ...
        //data----------------DT2
        detailSet.execute("SELECT d.* FROM " + tableName + " m LEFT JOIN " + tableName + "_DT2" + " d ON m.id = d.MAINID WHERE m.REQUESTID = " + requestId);
        while (detailSet.next()) {
            this.writeLog("DT2 执行==========");
            ...
            baoXiaoDetailVo.setAUFNR(detailSet.getString("nbdd"));//费用承担项目
            baoXiaoDetailVo.setZSE(detailSet.getString("se"));//税额
            baoXiaoDetailVo.setSGTXT(detailSet.getString("fph"));//发票号
            baoXiaoDetailVo.setWAERS(hb);//币种
            baoXiaoDetailVoList.add(baoXiaoDetailVo);
        }

        //data----------------DT3
        detailSet.execute("SELECT d.* FROM " + tableName + " m LEFT JOIN " + tableName + "_DT3" + " d ON m.id = d.MAINID WHERE m.REQUESTID = " + requestId);
        while (detailSet.next()) {
            this.writeLog("DT3 执行==========");
            if (detailSet.getString("je") == null || "".equals(detailSet.getString("je").trim())) {
                continue;
            }
            //明细表字段
        ...
        baseBean.writeLog("差旅报销拼接的json：" + TimeUtil.getCurrentTimeString() + " ---------- " + sendJson);

        //本地存储
        ActionUtil.saveDataJson(list);
        baseBean.writeLog("本地存储---------- ");
        //发送--------------------
        String code = ActionUtil.sendPo(sendJson);
        // String code = "11";
        baseBean.writeLog("推送返回状态码： " + code);
